    ...
    $(document).ready(function(){
		...
		$('form').on('submit', function(e) {
			...
			var isValidLogo = /(\.jpe?g|\.gif|\.png)$/i.test(i_logo.val()), 
				isValidImg = /(\.jpe?g|\.gif|\.png|\.mp4)$/i.test(i_img.val());
			if (!isValidLogo || !isValidImg) {
				$('.container_main .alert').remove();
				$('.container_main').prepend('<div class="alert alert-danger"><h4>No puede ser!</h4>Solo se admiten archivos JPG, GIF y PNG o MP4 para el contenido...</div>');
				$('html, body').animate({scrollTop:0}, 'slow');
				return false;
			} else {
		...
		$('button#btnPreview').click(function(){
			...
			mbody.find('#tpl_piepagina').removeClass('marquee').removeClass('down');
			
			
			mbody.find('#tpl_titulo').html($('form #titulo').val());
			mbody.find('#tpl_comentario').html($('form #comentario').val());
			mbody.find('#tpl_piepagina').html('<p>'+$('form #piepagina').val()+'</p>');
			
			if($('#optmarquee2').is(':checked')) {
				mbody.find('#tpl_piepagina').addClass('marquee').addClass('marqueeH');
			} else {
				if($('#optmarquee3').is(':checked')) {
					mbody.find('#tpl_piepagina').addClass('marquee').addClass('marqueeV');
				}
			}
